...
namespace RTMA {
	...
	public:
		Message( ) {
			_InstanceCount++;
			msg = new CMessage( );
			if( msg == NULL) throw gcnew Exception( "RTMA.Message(): Failed to allocate memory for CMessage");
...
#define CATCH_and_THROW_dotNET( ErrMessage) \
	...
	public:
		...
		Module( ) {
			mod = new RTMA_Module( );
			if( mod == NULL) throw gcnew Exception( "RTMA.Module(): Failed to allocate memory for RTMA_module");
		}
		~Module( ) { // Need descructor for garbage collection to work correctly
			delete mod;
		}
		// ConnectToMMM - Connect to the Message Manager Module, i.e. initiate communication with the RTMA system
		...
		// DisconnectFromMMM - Disconnect from the Message Manager Module, i.e. stop talking to the RTMA system
		...
		SendMessage( MSG_TYPE MessageType, Object ^Data, MODULE_ID DestModID, HOST_ID DestHostID, [OutAttribute]double %SendTime, [OutAttribute]MSG_COUNT %MessageCount) {
			TRY {
				...
				// Send the message
				if( !mod->SendMessageRTMA( &M, DestModID, DestHostID)) throw MyCException( "Failed to send the message");
...
/*
		...
		SendDD( MSG_TYPE MessageType, Object ^Data, MODULE_ID DestModID, HOST_ID DestHostID, [OutAttribute]double %SendTime, [OutAttribute]MSG_COUNT %MessageCount) {
			...
			} CATCH_and_THROW_dotNET( "Error in RTMA.Module.SendDD(): ")
		}
*/
		...
		SendSignal( MSG_TYPE MessageType, MODULE_ID DestModID, HOST_ID DestHostID, [OutAttribute]double %SendTime, [OutAttribute]MSG_COUNT %MessageCount) {
			TRY {
				...
				if( !mod->IsConnected( )) throw MyCException( "Module not connected to MM");
				CMessage M( MessageType);
				if( !mod->SendMessageRTMA( &M, DestModID, DestHostID)) throw MyCException( "Failed to send the signal");
				SendTime = M.send_time;
				MessageCount = M.msg_count;
		...
		ReadMessage( ReadType BlockingOrNot) {
			...
			} CATCH_and_THROW_dotNET( "Error in RTMA.Module.ReadMessage(ReadType BlockingOrNot): ")
		}
