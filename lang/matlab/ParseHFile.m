...
function [h, found_directive] = ParseNextItem( h, token, expected_directives)
    ...
    if( strcmp( token, 'using'))
        ...
        endidx = findstr(h.text,sprintf('\n'));
        h = consume_text(h,endidx);
        found_directive=[];
        return
    end
    if strcmp (token, 'class')
        NotYetImplemented('class');
        endidx = findstr(h.text,sprintf('};'))+1;
        h = consume_text(h,endidx);
        found_directive=[];
        return
    end
...
function h = ParseInclude( text, h)
    ...
    if( isempty( include))
%         error( 'Invalid filename in #include directive: %s',text);
            return
    end
...
function h = ParseDeclaration( h, typedef)
    ...
    if( isempty( decl))
        error( 'Type identifier not found in declaration'); 
    end
    h = consume_text( h, endidx);
    type = decl.type;
    switch( decl.type)
