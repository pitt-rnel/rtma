//
// A Matlab client interface to RTMA
//
// Meel Velliste
// March 9, 2006
//
...
// Op-codes for the MatlabRTMA mex function
#include "MatlabRTMA.h"

RTMA_Module TheModule;
CMessage M;

void Error(char *msg) {
  char str[10000];
  sprintf(str, "MatlabRTMA.cpp: %s", msg);
  mexErrMsgTxt(str);
}
    ...
    const mxArray *input_arg[]) // Array of right hand side arguments
{
  ...
  try {
    if (num_input_args < 1) {
      Error("MatlabRTMA takes at least 1 argument!");
    }
    ...
    case READ_MESSAGE_HDR:
      ...
      if (status > 0) {
        ...
        ReturnData = C2Matlab(Template, &M, sizeof(RTMA_MSG_HEADER));
    ...
    case READ_MESSAGE_DATA:
      ...
      Template = input_arg[1];
      ReturnData =
          C2Matlab(Template, M.GetDataPointer(), (size_t)M.num_data_bytes);
      output_arg[0] = ReturnData;
      break;
