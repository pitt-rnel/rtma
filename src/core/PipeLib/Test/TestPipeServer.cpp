...
public:
  ...
  void TestDisconnectingClients() { _server->DisconnectAllClients(); }
  ...
  void TestMultiRead_InOrder(void) {
    ...
    while (num_clients > 0) {
      ...
      std::cout << "Got data from " << num_clients_that_have_data << " clients"
                << std::endl;
  ...
  void TestWritingALot() {
    ...
    while (true) {
      ...
      // std::cout << "Value = " << value[0] << ", " << totalbytes << " bytes
      // written" << std::endl;
      value[0]++;
    }
    delete[] value;
...
// they want and exchanges some simple data with the clients
class MiniMessageManager {
private:
  UPipeServer *_server;

public:
  MiniMessageManager() { _server = NULL; }
  ~MiniMessageManager() {
    if (_server != NULL)
      delete _server;
  }
  ...
  void Run(void) {
    ...
    while (true) {
      ...
      if (num_ready > 0)
        std::cout << "Received data from " << num_ready << " clients"
                  << std::endl;
      ...
      for (int i = 0; i < num_ready; i++) {
        ...
        UPipe *p = pipes[r];
        std::cout << "Handling data from client #" << r << " (" << p << ")"
                  << std::endl;
        try {
          HandleData(p);
        } catch (UPipeClosedException) {
  ...
  void HandleData(UPipe *p) {
    ...
    p->Read(buffer, nbytes);
    std::cout << "Received data from " << p << ", a = " << data.a
              << ", b = " << data.b << ", x = " << data.x << std::endl;
    ...
    p->Write(buffer, nbytes);
    std::cout << "Sent response: a = " << data.a << ", b = " << data.b
              << ", x = " << data.x << std::endl;
  }
};
...
public:
  void HandleData(UPipe *pClientPipe) {
    ...
    std::cout << "Received data from " << pClientPipe << ", a = " << data.a
              << ", b = " << data.b << ", x = " << data.x << std::endl;
    // Send response to client
    double multiplier = (double)data.b;
    data.a = (int)_server->GetNumClients();
...
void TestServerNameParsing(void) {
  // To run this test, the private members of UPipeFactory need to be made
  // public, then the code below can be uncommented and compiled
  ...
  // gServerName, ServerName, PortNo); switch( t) { 	case
  // UPipeFactory::NamedPipe: printf("Named Pipe\n"); break; 	case
  // UPipeFactory::Socket: printf("Socket\n"); break; 	default:
  // printf("Hmmmm...\n");
  // }
}
