...
UPipeFactory::ServerType
UPipeFactory::ParseServerAddress(const char *server_addr, char *server_name,
                                 short *port_no) {
  TRY {
    int len = 0, i, start;
    ...
    if (len > MAX_SERVER_ADDR_LENGTH)
      throw UPipeException("Invalid server name, exceeds 512 characters");
    ...
    for (i = 0; i < len; i++) {
      ...
      if (server_addr[i] ==
          '/') { // if a named pipe in a specified Unix directory
    ...
    case Socket:
      ...
      while ((server_addr[i] != 0) && (i < len))
        i++;
      if ((server_addr[i] != 0) &&
          (i == len - 1)) { // we should have a terminating NULL to the
                            // string... if we don't- it's an error
        throw UPipeException(
            "Invalid server name, missing terminating null character");
        break;
      } else { // the port is in the string between start & i-1
        memcpy(str_port, server_addr + start + 1,
               (static_cast<size_t>(i) - 1) - start);
...
#ifdef _WINDOWS_C
      char pipe_dir[100] =
          "\\\\.\\pipe\\"; // required prefix for windows named pipe
#else
...
UPipeServer *UPipeFactory::CreateServer(char *server_addr) {
  TRY {
    ...
    if (!gRandSeeded) {
      ...
      gRandSeeded = true;
      // std::cout << "Random initialized, random number = " << rand() <<
      // std::endl;
    }

    if ((int)strlen(server_addr) > MAX_SERVER_ADDR_LENGTH)
      throw UPipeException("Server address string exceeds max length");
    ...
    case NamedPipe: {
      // #ifdef _UNIX_C
      // UnixNamedPipeServer *pNamedPipeServer = new UnixNamedPipeServer(
      // server_name); #else WinNamedPipeServer pNamedPipeServer = new
      // WinNamedPipeServer( server_name); #endif return (UPipeServer*)
      // pNamedPipeServer;
      throw UPipeException("Named pipes not yet implemented");
      break;
    }
    case Socket: {
      SocketPipeServer *pSocketServer =
          new SocketPipeServer(server_name, port_no);
...
UPipeClient *UPipeFactory::CreateClient(char *server_addr) {
  TRY {
    ...
    case NamedPipe: {
      // #ifdef _UNIX_C
      // UnixNamedPipeClient *pNamedPipeClient = new UnixNamedPipeClient(
      // server_name); #else WinNamedPipeClient pNamedPipeClient = new
      // WinNamedPipeClient( server_name); #endif return (UPipeClient*)
      // pNamedPipeClient;
      throw UPipeException("Named pipes not yet implemented");
      break;
    }
    case Socket: {
      SocketPipeClient *pSocketClient =
          new SocketPipeClient(server_name, port_no);
