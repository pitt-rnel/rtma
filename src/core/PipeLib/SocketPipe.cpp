...
// Create a TCP/IP stream socket
    ...
    if (strlen(host_addr) >
        0) { // If host name or IP specified, then use that specific address
      ...
      saServer->sin_addr =
          ((struct sockaddr_in *)result->ai_addr)->sin_addr; // Server's address
      freeaddrinfo(result);
    } else { // If no host name or IP specified, then allow any incoming
             // interface
...
#endif
      {
        throw UPipeClosedException(
            "Error while receiving data from the socket");
      }
...
/*
 * Disables the Nagle algorithm on the given socket to get rid of the 200 msec
 * delay in sending the packets
 */
static void DisableNagleAlgorithm(SocketHandle socket) {
  TRY {
    ...
    if (setsockopt(socket.id, p->p_proto, TCP_NODELAY, &one, sizeof(one)) < 0) {
      throw UPipeException("Could not set the TCP_NODELAY option to disable "
                           "the Nagle algorithm!");
    }
  }
 ...
 * Should get the same sockaddr_in *ServerAddr  that was used when creating the
 * listening socket
 */
static SocketHandle AcceptSocketClient(SocketHandle listening_socket) {
  TRY {
    ...
    // Wait for a client to connect: the accept call blocks until someone tries
    // to connect
    len = sizeof(struct sockaddr_in);
    accepted_socket.id =
        accept(listening_socket.id,
               (struct sockaddr *)(&(listening_socket.saServer)), &len);
  ...
  TRY {
...
#ifdef _WINDOWS_C
    int nRet =
        connect(socket.id, (LPSOCKADDR)saServer, sizeof(struct sockaddr));
#else
    int nRet = connect(socket.id, (struct sockaddr *)saServer,
...
#endif
    {
      throw UPipeException(
          "Could not connect to the socket: error in connect()");
    }
...
/*
 * Creates a TCP connection to the given [server:port] using global variable
 * theSocket Return: SOCKET_HANDLE on success, else throws UPipeException
 */
static SocketHandle OpenTCPConnection(char *ServerName, short nPort) {
  TRY {
    if (strlen(ServerName) == 0)
      throw UPipeException("ServerName is not allowed to be an empty string");
...
//
int SocketPipe::GetCapacity(void) {
  TRY {
...
int SocketPipe::Read(void *data_buffer, int nbytes_to_read, double timeout) {
  TRY {
    // Set up wait struct
    struct timeval wait{0, 0}, *pWait;
    if (timeout <
        0) { // Negative timeout value means we are willing to wait forever
      pWait = NULL;
    } else {
    ...
    if (status < 0) {
      int e = errno;
      // If a signal was caught, then it is not a real error but rather
      // something that just needs to be handled by some code above the level of
      // this code, so we throw a an exception to indicate that a signal was
      // received
      if (e == EINTR)
        throw UPipeSignalException(
            "A signal was caught while waiting for incoming data in select()");
      ...
      case EBADF: {
        ...
        if (used < sizeof(errmsg))
          snprintf(errmsg + used, sizeof(errmsg) - used,
                   " (EBADF, bad file descriptor in select set)");
        break;
      }
      // case EINTR: {
      //	size_t used = strlen(errmsg);
      //	if( used < sizeof(errmsg)) snprintf( errmsg + used,
      // sizeof(errmsg) - used, " (EINTR, select was interrupted by a signal)");
      //	break;
      // }
      case EINVAL: {
        size_t used = strlen(errmsg);
        if (used < sizeof(errmsg))
          snprintf(errmsg + used, sizeof(errmsg) - used,
                   " (EINVAL, nfds is negative or invalid timeout value)");
        break;
      }
      case ENOMEM: {
        size_t used = strlen(errmsg);
    ...
    if (FD_ISSET(_hPipe.id, &readfds)) {
      ...
      if (nbytes_read < nbytes_to_read)
        throw UPipeException("Failed to read data from pipe");
    }
...
int SocketPipe::Write(void *data_buffer, int n_bytes, double timeout) {
  TRY {
    // Set up wait struct
    ...
    if (FD_ISSET(_hPipe.id, &writefds)) {
      ...
      if (nbytes_writ != n_bytes)
        throw MyCException("Failed to write data to pipe");
    } else {
      // std::cout << "Socket could not accept data without blocking, data
      // discarded!" << std::endl;
      std::cout << "x";
    }
    return nbytes_writ;
...
#endif
  if (getpeername(_hPipe.id, (struct sockaddr *)&peer, &namelen) ==
      SOCKET_ERROR) {
    addr[0] = '\0';
    *port = 0;
    return 0;
...
SocketPipeServer::SocketPipeServer(char *host_addr, short port_no) {
  TRY {
...
#ifdef SO_REUSEPORT
    status = setsockopt(_hListeningSocket.id, SOL_SOCKET, SO_REUSEPORT, &optval,
                        sizeof(optval));
    if (status < 0) {
      CloseSocket(_hListeningSocket);
    ...
    Linger.l_onoff =
        1; /* enable linger option: If nonzero, close blocks until the data are
              transmitted or the timeout period has expired. */
    Linger.l_linger = 0; /* timeout in seconds */
    status = setsockopt(_hListeningSocket.id, SOL_SOCKET, SO_LINGER, &Linger,
                        sizeof(Linger));
    if (status < 0) {
      CloseSocket(_hListeningSocket);
    ...
    status = bind(_hListeningSocket.id,
                  (SOCKADDR *)(&(_hListeningSocket.saServer)), len);
    if (status == SOCKET_ERROR) {
      CloseSocket(_hListeningSocket);
    ...
    len = sizeof(struct sockaddr_in);
    status = ::bind(_hListeningSocket.id,
                    (struct sockaddr *)(&(_hListeningSocket.saServer)), len);
    if (status < 0) {
    ...
    // Specify a willingness to allow lots of connection requests to arrive at
    // the same time
...
//
int SocketPipeServer::WaitForDataAndClients(UPipe *pipes[], double timeout,
  ...
  TRY {
...
#ifdef _UNIX_C
      if (hClientPipe.id > high_fd)
        high_fd = hClientPipe.id;
#endif
    }
...
#ifdef _UNIX_C
      if (_hListeningSocket.id > high_fd)
        high_fd = _hListeningSocket.id;
#endif
    }
    ...
    if (*connection_request) {
      *connection_request =
          FD_ISSET(_hListeningSocket.id, &readfds) ? true : false;
    }
...
void SocketPipeServer::DoDisconnectClient(UPipe *client_pipe) {
  TRY {
    // Close the socket
...
UPipe *SocketPipeServer::DoAcceptClient(void) {
  TRY {
...
SocketPipeClient::SocketPipeClient(char *host_addr, short port_no) {
  TRY {
    ...
    if ((int)strlen(host_addr) > UPipeFactory::MAX_SERVER_ADDR_LENGTH)
      throw UPipeException("SocketPipeClient::SocketPipeClient(): Server name "
...
//
UPipe *SocketPipeClient::DoConnect(void) {
  TRY {
...
void SocketPipeClient::DoDisconnect(void) {
  TRY {
