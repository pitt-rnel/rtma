...
int MyCString::IsNull() const { return (m_Len == 0) ? 1 : 0; }

size_t MyCString::GetLen() const { return m_Len; }
...
void MyCString::AppendContent(const MyCString &str) {
  ...
  if (str.IsNull()) {
    ...
    strncpy(new_str + m_Len, str.GetContent(),
            str_len + 1); // append the new string & terminating NULL
...
void MyCString::AppendContent(const char *str, int len) {
  ...
  strncpy(new_str, m_Content,
          m_Len); // copy current string to new allocated string
  strncpy(new_str + m_Len, str,
          str_len); // append the new string & terminating NULL
...
char *MyCString::GetContent() const { return m_Content; }
...
void MyCString::DeletePrefix(const MyCString &delims) {
  ...
  for (itr = 0; itr < m_Len; itr++) {
    ...
    // no match was found && you reached to the end of the delims array? Then
    // this is a non delim char!
