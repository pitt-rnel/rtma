...
MyCException::MyCException(const char *err_str, int error_code)
    : m_TraceIndex(1), m_ErrorCode(error_code) {
  m_StackTrace[0].SetContent(err_str);
}

MyCException::MyCException(const MyCString &str, int error_code)
    : m_TraceIndex(1), m_ErrorCode(error_code) {
...
void MyCException::ReportToFile(const char *file_name) {
  ...
  // ReportFile << "--------------------------------------" << endl << "Stack
  // trace created " << __DATE__ << " " << __TIME__ << endl <<
  // "--------------------------------------" << endl;
