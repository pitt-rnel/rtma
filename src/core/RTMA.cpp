...
void *CMessage::GetDataPointer(void) {
  TRY {
    if (num_data_bytes > MAX_CONTIGUOUS_MESSAGE_DATA)
      return (void *)large_data;
    else
      return (void *)data;
  }
...
int CMessage::GetData(void *pData) const {
  TRY {
    if (num_data_bytes > 0) {
      if (num_data_bytes > MAX_CONTIGUOUS_MESSAGE_DATA) {
...
int CMessage::SetData(void *pData, int num_bytes) {
  TRY {
    if (num_bytes < 0)
      return 0;
    if (num_bytes > 0 && pData == NULL)
      return 0;

    if (!AllocateData(num_bytes))
      return 0;
...
int CMessage::AllocateData(int num_bytes) {
  TRY {
    if (num_bytes < 0)
      return 0;
...
int CMessage::Set(MSG_TYPE mt, void *pData, int num_bytes) {
  TRY {
    msg_type = mt;
...
int CMessage::Set(MSG_TYPE mt, const DD &dynamic_data) {
  TRY {
...
int CMessage::Set(MSG_TYPE mt, const std::string &dynamic_data) {
  TRY {
    // put the serialized data into CMessage's buffer
    // and set our message type
    int status = Set(mt, (void *)(dynamic_data.c_str()),
                     (int)dynamic_data.size() + 1); // add one to catch the null
...
int CMessage::Receive(UPipe *input_pipe)
// Receive a message from the pipe.
{
  TRY { return Receive(input_pipe, -1); }
  CATCH_and_THROW("CMessage::Receive( UPipe *input_pipe)");
}

int CMessage::Receive(UPipe *input_pipe, double timeout)
...
// If timeout is -1, then wait forever.
{
  TRY {
    ...
    if (num_data_bytes > 0) {
      ...
      if (num_data_bytes > MAX_CONTIGUOUS_MESSAGE_DATA) {
        if (!AllocateData(num_data_bytes))
          return -3;
        pData = large_data;
      }
...
int CMessage::Send(UPipe *output_pipe) {
  TRY { return Send(output_pipe, -1); }
  CATCH_and_THROW("CMessage::Send( UPipe *output_pipe)");
}

int CMessage::Send(UPipe *output_pipe, double timeout) {
  TRY {
    if (output_pipe == NULL)
      return 0;
    ...
    if (num_data_bytes <= MAX_CONTIGUOUS_MESSAGE_DATA) {
      ...
      if (nbytes_written < nbytes_to_send)
        success = 0;
      ...
      if (success) {
        ...
        payload_bytes_written =
            output_pipe->Write(large_data, nbytes_to_send, timeout);
        nbytes_written += payload_bytes_written;
        if (payload_bytes_written < nbytes_to_send)
          success = 0;
      }
    }
...
int RTMA_Module::ConnectToMMM(int logger_status, int read_dd_file,
                              int daemon_status)
// Opens a read and a write connection to the Message Management Module
{
  ...
  CATCH_and_THROW("RTMA_Module::ConnectToMMM( int logger_status, int "
                  "read_dd_file, int daemon_status)");
}

int RTMA_Module::ConnectToMMM(char *server_name, int logger_status,
                              int read_dd_file, int daemon_status)
// Opens a read and a write connection to the Message Management Module
{
  TRY {
    ...
    try {
      ...
      std::unique_ptr<CMessage> ack_msg(
          new CMessage()); // Use smart pointer for acknowledgment message
      status =
          WaitForAcknowledgement(1, ack_msg.get()); // Wait for up to 3 seconds
      if (status == 0) {
        throw MyCException(
            "Did not receive ACK from MessageManager upon CONNECT");
      }
      ...
      if (read_dd_file) {
        ...
        try {
          m_RTMA.Load(
              RTMA_DD_FILENAME,
              true); // will throw an exception if the file does not exist
        } catch (MyCException &E) {
          MyCString err;
          E.AppendTraceToString(err);
          CMessage R(MT_DYNAMIC_DD_READ_ERR, (void *)err.GetContent(),
                     err.GetLen());
          SendMessage(&R);
        }
      }
  ...
  CATCH_and_THROW("RTMA_Module::ConnectToMMM( char *server_name, int "
                  "logger_status, int read_dd_file, int daemon_status)");
}

int RTMA_Module::DisconnectFromMMM(void) {
  TRY {
    DEBUG_TEXT_("DisconnectFromMMM():");
    if (m_Connected) {
      try {
        // Try to disconnect cleanly at RTMA level
        SendSignal(MT_DISCONNECT); // this could go to a dead pipe- but it won't
                                   // be blocking
      } catch (...) {
    ...
    if (_pipeClient != NULL) {
      try {
        ...
        // If clean disconnect fails then we still need to release the client
        // object
      }
...
int RTMA_Module::IsConnected(void) {
  TRY {
    if (!m_Connected) {
...
int RTMA_Module::SendModuleReady(void) {
  TRY {
...
int RTMA_Module::Subscribe(MSG_TYPE MessageType)
// MessageType is a specific message ID to subscribe just to that type of
// message, or ALL_MESSAGE_TYPES to get all message types (useful for
// debugging);
{
  TRY {
...
int RTMA_Module::Unsubscribe(MSG_TYPE MessageType)
// MessageType is a specific message ID to unsubscribe from only that message
// type or ALL_MESSAGE_TYPES to unsubscribe from all message types
{
  TRY {
...
int RTMA_Module::PauseSubscription(MSG_TYPE MessageType)
...
int RTMA_Module::ResumeSubscription(MSG_TYPE MessageType)
// Tell MessageManager to resume sending messages of a specific MessageType,
// or ALL_MESSAGE_TYPES after having paused those message types using
// PauseSubscription
{
  TRY {
...
int RTMA_Module::WaitForAcknowledgement(double timeout, CMessage *rcvMsg) {
  TRY {
    int ret = 0;
    ...
    if (timeout == -1) { // Wait forever
      ...
      while (time_remaining > 0) {
        ...
        if (status == 0)
          break;
        if (status < 0)
          throw MyCException("Error while waiting for MT_ACKNOWLEDGE");
...
int RTMA_Module::SendMessage(CMessage *M, UPipe *output_pipe,
                             MODULE_ID dest_mod_id, HOST_ID dest_host_id) {
  TRY {
    if (output_pipe == NULL)
      return 0;
  ...
  CATCH_and_THROW("RTMA_Module::SendMessage( CMessage *M, UPipe *output_pipe, "
                  "MODULE_ID dest_mod_id, HOST_ID dest_host_id)");
}

int RTMA_Module::SendMessage(CMessage *M, MODULE_ID dest_mod_id,
                             HOST_ID dest_host_id) {
  TRY { return SendMessage(M, _MMpipe, dest_mod_id, dest_host_id); }
  CATCH_and_THROW("RTMA_Module::SendMessage( CMessage *M, MODULE_ID "
                  "dest_mod_id, HOST_ID dest_host_id)");
}
int RTMA_Module::SendMessageRTMA(CMessage *M, MODULE_ID dest_mod_id,
                                 HOST_ID dest_host_id) {
  TRY { return SendMessage(M, _MMpipe, dest_mod_id, dest_host_id); }
  CATCH_and_THROW("RTMA_Module::SendMessageRTMA( CMessage *M, MODULE_ID "
                  "dest_mod_id, HOST_ID dest_host_id)");
}

#ifdef USE_DYNAMIC_DATA
int RTMA_Module::SendMessage(MSG_TYPE mt, const DD &dynamic_data,
                             MODULE_ID dest_mod_id, HOST_ID dest_host_id) {
  TRY {
    CMessage M;
  ...
  CATCH_and_THROW("RTMA_Module::SendMessage(MSG_TYPE mt, const DD& "
                  "dynamic_data, MODULE_ID dest_mod_id, HOST_ID dest_host_id)");
}
#endif

int RTMA_Module::SendSignal(MSG_TYPE MessageType, UPipe *output_pipe,
                            MODULE_ID dest_mod_id, HOST_ID dest_host_id)
// Send message that only has the header (no data after the header).
{
  ...
  CATCH_and_THROW("RTMA_Module::SendSignal( MSG_TYPE MessageType, UPipe "
                  "*output_pipe, MODULE_ID dest_mod_id, HOST_ID dest_host_id)");
}

int RTMA_Module::SendSignal(MSG_TYPE MessageType, MODULE_ID dest_mod_id,
                            HOST_ID dest_host_id)
// Send message that only has the header (no data after the header).
{
  TRY {
  ...
  CATCH_and_THROW("RTMA_Module::SendSignal( MSG_TYPE MessageType, MODULE_ID "
                  "dest_mod_id, HOST_ID dest_host_id)");
}

int RTMA_Module::SendSelfSignal(MSG_TYPE sig)
// Sends signal this this module itself
{
  TRY { return SendSignal(sig, m_ModuleID, m_HostID); }
  CATCH_and_THROW("RTMA_Module::SendSelfSignal( MSG_TYPE sig)");
}

int RTMA_Module::NBReadMessage(CMessage *M) {
  TRY { return ReadMessage(M, 0); }
  CATCH_and_THROW("RTMA_Module::NBReadMessage( CMessage *M)");
}

int RTMA_Module::ReadMessage(CMessage *M, double timeout) {
  TRY {
    int status;

    status = M->Receive(_MMpipe, timeout);
    if (status < 0)
      throw MyCException("Failed to read message");
...
int RTMA_Module::WaitForSignal(MSG_TYPE SigType, int blocking)
// Waits for SigType message: if blocking- will not return until the requested
// sig type was received. If non blocking- will return 1 if the requested type
// was received, or 0 if another type was received
{
  TRY {
    CMessage M;
...
void RTMA_Module::WaitForMessage(CMessage *M, MSG_TYPE MsgType)
// Waits for a message:
// if MsgType is specified- will not return until the requested msg type was
// received. if MsgType is not specified- will return the first message received
// (in this case just a wrapper for ReadMessage)
{
  TRY {
    while (1) {
      ReadMessage(M);
...
int RTMA_Module::IncrementMessageCount()
...
double RTMA_Module::UpTime(void) const {
  TRY { return GetAbsTime() - m_StartTime; }
  CATCH_and_THROW("RTMA_Module::UpTime( void)");
}

int RTMA_Module::GetPid(void) const {
  TRY { return m_Pid; }
  CATCH_and_THROW("RTMA_Module::GetPid( void)");
}

/*
 * Opens a handle to this process (specified by m_Pid), and sets the priority
 * class accordingly (Does not set priority for all threads - just for the main
 * thread)
 *
 * Returns 1 on success, 0 on failure
 */
int SetMyPriority(int priority_class) {
  /*
  TRY {
#ifdef _UNIX_C
          //sched_setscheduler
          return 1;
#else
          HANDLE hProcess = GetCurrentProcess();//not need to close this pseudo
handle if(hProcess == NULL) return 0;
...
#else
    HANDLE hProcess =
        GetCurrentProcess(); // not need to close this pseudo handle
    if (hProcess == NULL)
      return 0;
