...
mxArray *EmptyMatrix(void) { return mxCreateDoubleMatrix(0, 0, mxREAL); }
    ...
    const mxArray *input_arg[]) // Array of right hand side arguments
{
  ...
  try {
    ...
    if (num_input_args < 1)
      throw MyCException("MatlabRTMA takes at least 1 argument (an op-code)!");
    ...
    case READ_HEADERS:
      ...
      if (num_input_args < 2)
        throw MyCException("Missing header template argument");
    ...
    case READ_DATA_BLOCK:
      ...
      if (num_input_args < 2)
        throw MyCException(
            "Missing data template and data block index arguments");
      if (num_input_args < 3)
        throw MyCException("Missing data block index argument");
      if (num_input_args < 4)
        throw MyCException("Missing number of data bytes argument");
      DataTemplate = input_arg[1];
      DataBlockIndex = (unsigned int)mxGetScalar(input_arg[2]);
      NumDataBytes = (int)mxGetScalar(input_arg[3]);
      NumMessages = MB->GetNumMessages();
      if (DataBlockIndex >= NumMessages)
        throw MyCException("DataBlockIndex exceeds number of messages");
    ...
    case GET_NUM_BYTES:
      ...
      if (num_input_args < 2)
        throw MyCException("Missing data template argument");
      DataTemplate = input_arg[1];
