// This file may be of historical interest. It represents the original rough
// design of RTMA, written down as pseudo-code
...
MessageManger is Module 0
    ...
    typedef struct {
  msg_count;   // Source message count (per source, starting from 1 when source
               // connects to MMM)
  send_time;   // Time at source when message sent (Seconds since 0:00:00 on Jan
               // 1, 1970)
  recv_time;   // Time at final destination when message received (Seconds since
               // 0:00:00 on Jan 1, 1970)
  ...
  dest_host_id;   // ID number of the host computer intended to receive this
                  // message
...
// This means that two modules should never share the same address space if
// using C.
MMPipeHandle ModulePipeHandle ThisModuleID MessageCount
...
SendMessage(Message) {
  Assume that msg_type and data have been filled in Fill in the rest of the
      header Take care to write the entire message to MMPipeHandle in one write
          call

              MessageCount++;
}

ReadMessage()->Message, Status{Read sizeof(MESSAGE_HEADER)
                                   bytes from ModulePipeHandle Get
                                       num_data_bytes from header Read
                                           num_data_bytes from ModulePipeHandle}
...
ProcessMessage(M)->SubscriberList {
  ...
  switch (M.msg_type) {
  case CONNECT:
    ConnectModule(M.src_mod_id);
  case DISCONNECT:
    DisconnectModule(M.src_mod_id);
  case SUBSCRIBE:
    AddSubscription(M.src_mod_id, M.data.desired_msg_type);
  case UNSUBSCRIBE:
    RemoveSubscription(M.src_mod_id, M.data.desired_msg_type);
  default:
    SubscriberList = GetSubscriberList(M.msg_type);
  }
}

// clang-format off
ConnectModule(module_id){
  Create module record Open write handle to module's pipe Keep module's pipe handle in module record Send ACK to module}

      DisconnectModule(module_id){
          Close handle to
              module's pipe Delete module record Cancel all the subscriptions of
                  the module Send ACK to module}

  AddSubscription(module_id, message_type) {
    if a
      particular message_type Add module entry in
          SubscribersToMessageType[message_type];
  ...
  RemoveSubscription(module_id, message_type) {
    if a
      particular message_type Find and remove the module's entry in SubscribersToMessageType[message_type]; if ALL_MESSAGE_TYPES Find
          and remove the
              module's entries in SubscribersToMessageType[*]; Send ACK to module(
                  unless called from DisconnectModule)
  }
