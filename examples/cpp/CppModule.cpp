...
void TestProducer(char *server_address) {
  ...
  if (strlen(server_address) > 0)
    mod.ConnectToMMM(server_address);
  else
    mod.ConnectToMMM();
...
void TestConsumer(char *server_address) {
  ...
  while (run) {
    ...
    case MT_TEST_DATA:
      M.GetData(&data);
      std::cout << "Data = [a: " << data.a << ", b: " << data.b
                << ", x: " << data.x << "]" << std::endl;
      break;
    case MT_EXIT:
    case MT_KILL:
...
int main(int argc, char *argv[]) {
  try {
    ...
    if (argc < 2)
      throw MyCException("Need a command-line argument that specifies "
                         "'producer' or 'consumer'");
    char *option = argv[1];
    if (strcmp(option, "producer") == 0) {
      if (argc > 2)
        TestProducer(argv[2]); // If server address specified, then use it
      else
        TestProducer("");
    } else if (strcmp(option, "consumer") == 0) {
      if (argc > 2)
        TestConsumer(argv[2]); // If server address specified, then use it
      else
        TestConsumer("");
    } else {
      throw MyCException(
          "Command-line argument must be 'producer' or 'consumer'");
    }
