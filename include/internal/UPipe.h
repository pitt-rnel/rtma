...
public:
  ...
  UPipeException(const char *err_str, int error_code = 0)
      : MyCException(err_str, error_code) {}
  UPipeException(MyCString &str, int error_code = 0)
      : MyCException(str, error_code) {}
};

class UPipeClosedException : public UPipeException {
public:
  UPipeClosedException() : UPipeException() {}
  UPipeClosedException(const char *err_str, int error_code = 0)
      : UPipeException(err_str, error_code) {}
  UPipeClosedException(MyCString &str, int error_code = 0)
      : UPipeException(str, error_code) {}
};

class UPipeSignalException : public UPipeException {
public:
  UPipeSignalException() : UPipeException() {}
  UPipeSignalException(const char *err_str, int error_code = 0)
      : UPipeException(err_str, error_code) {}
  UPipeSignalException(MyCString &str, int error_code = 0)
      : UPipeException(str, error_code) {}
};

// short-hand for try and catch that can be put in every single function so as
// to build a stack trace for each error that occurs
#define TRY try
...
public:
  ...
  void Read(void *data_buffer, int n_bytes) {
    TRY { Read(data_buffer, n_bytes, -1); }
  ...
  // Makes a blocking call to write n_bytes from data_buffer to the pipe. Does
  // not return until the requested number of bytes have been written. Returns
  // number of bytes written.
  int Write(void *data_buffer, int n_bytes) {
    TRY { return Write(data_buffer, n_bytes, -1); }
  ...
  // return until enough bytes are read out from the other end that the write
  // call can complete writing all the data. If timeout is -1, waits
  // indefinitely. If a timeout in seconds is given, only waits that long. If
  // timeout is 0, then never blocks (if no space in pipe, then returns with 0).
...
// Implements a simple set of methods for running a pipe server. Do not derive
// from this class in order to use it. Instead, create an instance using the
// UPipeFactory::CreateServer() method. This is because UPipeServer is an
// abstract class that allows derived classes to implement its functionality for
// any type of underlying pipe-like communication method. For example, a derived
// class provided with this library implements it with standard sockets.
class UPipeServer {
public:
  ...
  // constructor
  UPipeServer() { Initialize(); }

  unsigned int GetNumClients() const { return _numClients; }

  // Accepts a client from the listening socket and returns a pointer to a pipe
  // object that represents the new connection. This pipe object can be used to
  // excange data with the client. If a connection request has not yet been
  // received, then this call blocks until a connection request comes in. The
  // WaitForDataAndClients method can be used to make sure there is a request
  // before calling this method.
  UPipe *AcceptClient(void) {
    TRY {
      // Check to make sure we are not going to exceed the number of allowed
      // connections
      if (_numClients == MAX_CLIENTS)
        throw UPipeException(
            "Cannot accept another client, max number of clients reached");
  ...
  // Disconnects the client connected through client_pipe and deletes the
  // client_pipe object
  void DisconnectClient(UPipe *client_pipe) {
    TRY {
      // Make sure it's not a NULL pointer
      if (client_pipe == NULL)
        throw UPipeException("client_pipe is a NULL pointer");
  ...
  // Waits for data on all connected client sockets and optionally also waits
  // for connection requests on the listening socket all in one blocking call,
  // eliminating the need for a separate thread for listening for clients.
  // "pipes" and "timeout" parameters and the return value behave the same as in
  // the WaitForData method. If "connection_request" is true on input, then
  // waits for connection requests. On output, "connection_request" is set to
  // true if a connection request from a new client has been received and is set
  // to false otherwise. If there is a connection request, it can be accepted
  // using the AcceptClient method.
  virtual int WaitForDataAndClients(UPipe *pipes[], double timeout,
                                    bool *connection_request) = 0;

  // Returns a uniformly distributed random integer between 0 and N-1. N can be
  // at most RAND_MAX. This is intended as a helper method for picking a random
  // index to choose a random one of the pipes returned by WaitForData() that
  // have data ready to be read.
  static int RandomIndex(int N) {
    TRY {
      // Here we do not use the common pattern of turning the raw random into a
      // floating point number, rescaling it and then truncating the result to
      // an int, because that would result in quantizing the rescaled values and
      // would result in non-uniform random int values, especially for higher
      // values of N.
      //
      // So, instead we keep it in integer space and do a modulo to get
      // uniformly distributed ints. If the generated raw number is from the
      // tail end of the distribution, then we draw a new random number because
      // the tail end of the distribution cannot return all numbers 0..N-1
      // unless RAND_MAX+1 is divisible by N.
      int maxN = RAND_MAX;
      if (N > maxN)
        throw UPipeException(
            "trying to get a random int greater than RAND_MAX");
      int r = rand();
      while (r >= (maxN - (maxN % N)))
        r = rand();
      return r % N;
  ...
  // Returns an array of random indices from 0 to N-1 so that each index occurs
  // exactly once, where N can be at most RAND_MAX (i.e. 32768 in the current
  // C-library at the time of this writing). This is intended as a helper method
  // for processing the returned data pipes from WaitForData() in random order.
  // "indices" must be pre-allocated with at least N elements.
  static void RandomIndices(int N, int *indices) {
    TRY {
      // If N is less than 1, then there are no indices to assign
      if (N < 1)
        return;
      // Special but frequent case to avoid unnecessary calls to random
      // generator
      if (N == 1) {
        indices[0] = 0;
        return;
...
protected:
  // Accepts a connection request from a client and returns a pointer to a pipe
  // object that represents the server end of the connected pipe.
...
protected:
  ...
  // Remove a client pipe pointer from client list. Returns true if client was
  // removed from the list or false when client was not found in the list.
  bool RemoveClientFromList(UPipe *pClientPipe) {
    TRY {
      // Find client in the list
      unsigned int i;
      for (i = 0; i < _numClients; i++)
        if (_clientList[i] == pClientPipe)
          break;
      if (i == _numClients)
        return false; // If client not in list
...
public:
  ...
  // If server_addr contains a ":" then the string before the colon is treated
  // as a hostname or IP address and the string after colon is port number. If
  // there is no colon then server_addr is treated as a local pipe name. On
  // Unix, if a local pipe, the system will look for the file-system stub for
  // the pipe in /tmp by default unless the name contains "/" characters in
  // which case the specifid path is used.
...
private:
  ...
  typedef enum { NamedPipe, Socket } ServerType;
  static ServerType ParseServerAddress(const char *server_addr,
                                       char *server_name, short *port_no);
};
...
// Implements a simple server that allows clients to connect/disconnect whenever
// they want and provides an overridable method for derived classes to implement
// data handling. In other words, takes all the hassle out of writing a pipe
// server, leaving the derived class only to decide how it wants to respond when
// data arrives from any one client. Run the auto-server by calling the Run()
// method which never returns. Inside the Run() method it implements accpepting
// client connections and waits for data from existing clients. When data
// becomes available from a client, calls the HandleData() method that needs to
// be overridden by a derived class. The Run() method also implements clean
// handling of unexpected client-side disconnect or crash. The implementation of
// this class is quite simple, so it's provided directly here in the h-file, to
// make it easy to see what actually happens.
...
public:
  ...
  double GetLatestRecvTime(void) { return _server->_recvTime; }
  ...
  void Run(char *ServerName) {
    ...
    while (_keepRunning) {
      ...
      int num_ready =
          _server->WaitForDataAndClients(pipes, -1, &connection_request);

      // Handle the data (in random order if we got data from more than one
      // client at once)
